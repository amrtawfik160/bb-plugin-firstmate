...
- The native per-task commit-message hook (`state/<id>.git-hooks`, installed by `fm-spawn.sh` through `fm-git-strip-ai-trailers.sh`) is created and torn down for BB crews but inert: native activates it by exporting `core.hooksPath` in the pane launch command, and BB threads never receive that export (the agent is launched at thread creation and `export` lines are dropped). AI commit trailers are not stripped at the commit object for BB crews. Follow-up: deliver `core.hooksPath` to BB threads.
- No Escape key.
