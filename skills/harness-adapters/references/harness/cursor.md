...
| Attribution | Cursor can append a Co-authored-by trailer after the typed message. Fleet launches install the per-task commit-msg strip through `fm-git-strip-ai-trailers.sh`; it removes known AI trailers while preserving human co-authors and the author identity. |
